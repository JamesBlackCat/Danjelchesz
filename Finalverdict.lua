...
-- ============================================================
--  CONFIG SAVES  (writefile / readfile via executor APIs)
-- ============================================================
local HS        = game:GetService("HttpService")
local SAVE_FILE = "AimLua_saves.json"

-- Executor file I/O — wrapped so missing APIs fail gracefully
local function _readFile(path)
    if not pcall(function() return type(readfile) end) then return nil end
    local ok, data = pcall(readfile, path)
    return ok and data or nil
end
local function _writeFile(path, data)
    if not pcall(function() return type(writefile) end) then return end
    pcall(writefile, path, data)
end

-- Read the saves store from disk; returns {} if missing or corrupt
local function readSaves()
    local raw = _readFile(SAVE_FILE)
    if not raw or raw == "" then return {} end
    local ok, t = pcall(function() return HS:JSONDecode(raw) end)
    return (ok and type(t) == "table") and t or {}
end

-- Write the saves store back to disk
local function writeSaves(t)
    local ok, encoded = pcall(function() return HS:JSONEncode(t) end)
    if ok then _writeFile(SAVE_FILE, encoded) end
end

-- Snapshot current S into a plain JSON-safe table.
-- Primitives (number/bool/string) from D are included automatically.
-- UDim2 crosshair pos and KeyCode are handled specially.
local function serializeS()
    local out = {}
    ...
    -- Crosshair position (UDim2)
    if S.aimlockCrosshairPos then
        out._crosshairX = S.aimlockCrosshairPos.X.Scale
        out._crosshairY = S.aimlockCrosshairPos.Y.Scale
    end
    -- Keybind (Enum.KeyCode)
    out._aimlockKeyName = S.aimlockKey and S.aimlockKey.Name or "P"
    return out
end

-- Apply a deserialized config back onto S and trigger immediate side-effects.
local function applyConfig(data)
    -- Restore primitives
    for k, v in pairs(data) do
        local t = type(v)
        if k == "_crosshairX" then
            S.aimlockCrosshairPos = UDim2.new(
                v, 0, S.aimlockCrosshairPos.Y.Scale, 0)
        elseif k == "_crosshairY" then
            S.aimlockCrosshairPos = UDim2.new(
                S.aimlockCrosshairPos.X.Scale, 0, v, 0)
        elseif k == "_aimlockKeyName" then
            local ok2, kc = pcall(function() return Enum.KeyCode[v] end)
            if ok2 and kc then S.aimlockKey = kc end
        elseif (D[k] ~= nil) and (t == "number" or t == "boolean" or t == "string") then
            S[k] = v
        end
    end
    -- Immediate side-effects so behaviour matches right away
    if Aimlock.fovCircle then
        Aimlock.fovCircle.Visible = S.aimlockShowFOV
    end
    if Aimlock.fovStroke then
        Aimlock.fovStroke.Color = Color3.fromRGB(
            S.aimlockFOVColorR, S.aimlockFOVColorG, S.aimlockFOVColorB)
    end
    if not S.espEnabled then pcall(clearAllESPBoxes) end
    -- Crosshair position
    local function movePiece(p)
        if p then p.Position = S.aimlockCrosshairPos end
    end
    movePiece(Aimlock.crosshairDot)
    movePiece(Aimlock.crosshairCrossH)
    movePiece(Aimlock.crosshairCrossV)
end
...
-- ── SAVES ─────────────────────────────────────────────────────
local TSave = Window:CreateTab("Saves", 4483362458)

-- Forward-declared dropdown handles so refresh helpers can see them
local _loadDrop, _deleteDrop

-- Rebuilds both dropdowns to match current saves on disk
local function refreshSaveDropdowns()
    local saves = readSaves()
    local names = {}
    for name in pairs(saves) do table.insert(names, name) end
    table.sort(names)
    local display = #names > 0 and names or {"(no saves yet)"}
    pcall(function() _loadDrop:Refresh(display,   {}) end)
    pcall(function() _deleteDrop:Refresh(display, {}) end)
end

-- ── Save section ──────────────────────────────────────────────
TSave:CreateSection("Save  —  name your config, then press Save")
local _saveName = ""
TSave:CreateInput({
    Name                   = "Config Name",
    PlaceholderText        = "e.g.  headshot build",
    RemoveTextAfterFocusLost = false,
    Flag                   = "SaveConfigName",
    Callback               = function(v) _saveName = v or "" end,
})
TSave:CreateButton({
    Name     = "Save Current Config",
    Callback = function()
        local name = _saveName:match("^%s*(.-)%s*$")   -- trim whitespace
        if name == "" then
            Rayfield:Notify({
                Title    = "Save",
                Content  = "Type a name above before saving.",
                Duration = 3,
            })
            return
        end
        local saves = readSaves()
        saves[name]  = serializeS()
        writeSaves(saves)
        refreshSaveDropdowns()
        Rayfield:Notify({
            Title    = "Saved ✓",
            Content  = "'" .. name .. "' saved.",
            Duration = 3,
...
-- ── Load section ──────────────────────────────────────────────
TSave:CreateSection("Load  —  tap a save to apply it instantly")
_loadDrop = TSave:CreateDropdown({
    Name           = "Saved Configs",
    Options        = {"(no saves yet)"},
    CurrentOption  = {},
    MultipleOptions = false,
    Flag           = "LoadConfigDropdown",
    Callback       = function(v)
        local name = (type(v) == "table" and v[1]) or v
        if name == "(no saves yet)" then return end
        local saves = readSaves()
        if not saves[name] then
            Rayfield:Notify({
                Title   = "Load",
                Content = "'" .. name .. "' not found on disk.",
                Duration = 3,
            })
            return
        end
        applyConfig(saves[name])
        Rayfield:Notify({
            Title    = "Loaded ✓",
            Content  = "'" .. name .. "' applied.
Sliders update on next open.",
            Duration = 5,
        })
    end,
})
...
-- ── Delete section ─────────────────────────────────────────────
TSave:CreateSection("Delete  —  select a save, then press Delete")
local _deleteTarget = ""
_deleteDrop = TSave:CreateDropdown({
    Name           = "Select Save to Delete",
    Options        = {"(no saves yet)"},
    CurrentOption  = {},
    MultipleOptions = false,
    Flag           = "DeleteConfigDropdown",
    Callback       = function(v)
        _deleteTarget = (type(v) == "table" and v[1]) or v
    end,
})
TSave:CreateButton({
    Name     = "Delete Selected Save",
    Callback = function()
        if _deleteTarget == "" or _deleteTarget == "(no saves yet)" then
            Rayfield:Notify({
                Title    = "Delete",
                Content  = "Select a save from the list first.",
                Duration = 3,
            })
            return
        end
        local saves = readSaves()
        saves[_deleteTarget] = nil
        writeSaves(saves)
        _deleteTarget = ""
        refreshSaveDropdowns()
        Rayfield:Notify({
            Title    = "Deleted",
            Content  = "Save removed.",
            Duration = 3,
        })
    end,
})

-- Populate both dropdowns from disk at startup
refreshSaveDropdowns()
