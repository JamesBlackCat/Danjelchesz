...
local D = {
    ...
    -- ── Target Mode (independent toggles) ────────────────
    targetPlayers             = true,       -- target players
    targetNPCs                = false,      -- target NPC humanoid models
    -- ── Images ────────────────────────────────────────────
...
local Aimlock = {
    ...
    targetLostAt    = 0,
    crosshairGui    = nil,
    crosshairDot    = nil,   -- circle dot frame
    crosshairCrossH = nil,   -- cross horizontal bar
    crosshairCrossV = nil,   -- cross vertical bar
    fovCircle       = nil,
    fovStroke       = nil,
    statusLabel     = nil,
    friendCache     = {},
...
-- ── NPC scan — returns all non-player humanoid models ─────────
...
-- ── Candidate list builder (respects independent toggles) ─────
...
-- Targets inside the FOV ring come first (sorted by screen distance),
-- then targets outside FOV (sorted by world distance).
            ...
            if part then
                ...
                if onScreen and sp.Z > 0 then
                    ...
                    if screenD2 <= fovR2 then
                        ...
                        table.insert(outFov, { entity = entity, dist = worldD })
                    end
                else
                    local worldD = (part.Position - hrp.Position).Magnitude
                    table.insert(outFov, { entity = entity, dist = worldD })
...
local function makeCrosshairGui()
    ...
    chStroke.Transparency = 0.25
    chStroke.Parent       = crossH
    ...
    cvStroke.Transparency = 0.25
    cvStroke.Parent       = crossV
    Aimlock.crosshairCrossV = crossV

    -- Apply initial shape visibility
    syncCrosshairShape()
...
RunService:BindToRenderStep("FlyAimlock", Enum.RenderPriority.Camera.Value + 2, function()
    ...
    -- Sync shape visibility each frame (catches runtime toggles)
    syncCrosshairShape()
...
-- ── Target mode buttons (PLR / NPC — independent toggles) ─────
local modeColors = {
...
-- PLR button — toggles S.targetPlayers
local plrBtnObj = makeButton("ModeBtn_player", S.modeBtnPlayerPos, S.btnSize, "PLR",
    modeColors.active,   -- players on by default
    function()
        S.targetPlayers = not S.targetPlayers
        refreshModeButtons()
    end)
...
-- Initialise button tints to match defaults
refreshModeButtons()
...
RunService.Heartbeat:Connect(function()
    ...
    -- Mode buttons (shown when either mode is active via Rayfield toggle)
    local showMode = S.targetPlayers ~= nil   -- always show once enabled via Rayfield
    plrBtnObj.frame.Visible = S._showModeBtns == true
    npcBtnObj.frame.Visible = S._showModeBtns == true
    -- Aimlock button always visible
...
-- Target Mode — PLR and NPC are independent toggles
-- On-screen PLR/NPC buttons appear when this section's master toggle is on.
...
TVis:CreateToggle({
    Name         = "Cross Crosshair  (replaces dot with + shape)",
    CurrentValue = S.aimlockCrosshairShape == "cross",
    Flag         = "CrosshairCross",
    Callback     = function(v)
...
TVis:CreateSlider({
    ...
    Callback     = function(v)
        ...
        -- apply to whichever shape is visible
        local t = v / 100
...
-- Keep dropdowns in sync as players join / leave
...
Players.PlayerRemoving:Connect(function()
    task.wait(0.1)   -- brief delay so the player is actually gone from GetPlayers()
