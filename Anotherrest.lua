...
-- Called every frame from BindToRenderStep
local function updateESPBoxes()
    if not S.espEnabled then
        -- ESP off: disable all GUIs immediately
    ...
    for model in pairs(npcCache) do
        ...
        if not model.Parent then
            ...
            if root and alive then
                ...
                if inRange then
                    -- Lazily create BillboardGui (once per NPC)
                    if not espBoxes[model] then
                        ...
                        -- Name label (top strip, 18 px tall)
                        ...
                        lbl.Visible                 = S.espNameTag == true
                        lbl.Parent                  = gui

                        -- Colored square (below the label)
                        ...
                        -- Black outline — fixed color, cannot be changed
                    ...
                    local e = espBoxes[model]
                    e.gui.Enabled            = true
                    e.frame.BackgroundColor3 = boxCol
                    if e.lbl then
                        e.lbl.TextColor3 = boxCol
                        e.lbl.Visible    = S.espNameTag == true
                    end
                else
                    -- Out of range — disable GUI (keep object to avoid GC churn)
                    local e = espBoxes[model]
                    if e and e.gui then e.gui.Enabled = false end
                end
            else
                -- Dead or rootless — destroy immediately
                removeESPBox(model)
            end
        end
    end

    -- Destroy boxes for models that left the cache since last frame
    for model in pairs(espBoxes) do
