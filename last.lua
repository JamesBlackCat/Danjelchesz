...
RunService:BindToRenderStep("FlyAimlock", Enum.RenderPriority.Camera.Value + 2, function()
    ...
    if S.switchTargetsEnabled then
        local targetInvalid = Aimlock.target and not isEntityAlive(Aimlock.target)
        if Aimlock.target and isDistanceStickyEnabled()
        and Aimlock.targetModel ~= getEntityModel(Aimlock.target) then
            targetInvalid = true
        end
        if targetInvalid and isDistanceStickyEnabled() then
        ...
        if cur and (S.aimlockStickyLock or isDistanceStickyEnabled()) then
            if isDistanceStickyEnabled() then
                -- Distance sticky mode ignores FOV, range, wall, and
                -- reacquire checks after the initial target is acquired.
