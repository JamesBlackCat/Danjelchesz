...
local function isValidPlayerTarget(plr, requireInFOV, cross)
    ...
    if requireInFOV and cross then
        ...
        if (dx * dx + dy * dy) > (fovRadius * fovRadius) then return false end
    end
    return true
end
...
local function enterCrosshairEditMode()
    ...
    local function setMode(nextMode)
        ...
        handle.BackgroundColor3 = mode == "drag"
            and Color3.fromRGB(255, 80, 80)
            or Color3.fromRGB(80, 130, 220)
    end

    local function nudge(dx, dy)
        local pos = S.aimlockCrosshairPos
        applyCrosshairPosition(UDim2.new(
            math.clamp(pos.X.Scale + dx, 0, 1), 0,
            math.clamp(pos.Y.Scale + dy, 0, 1), 0))
        syncHandle()
    end
    ...
    makeEditorButton("Up", UDim2.new(0.08, 0, arrowY, 0),
        Color3.fromRGB(70, 70, 70), function() if mode == "move" then nudge(0, -0.01) end end)
    makeEditorButton("Down", UDim2.new(0.30, 0, arrowY, 0),
        Color3.fromRGB(70, 70, 70), function() if mode == "move" then nudge(0, 0.01) end end)
    makeEditorButton("Left", UDim2.new(0.52, 0, arrowY, 0),
        Color3.fromRGB(70, 70, 70), function() if mode == "move" then nudge(-0.01, 0) end end)
